...
export function validateExerciseItem(
  item: ExerciseItem,
  attempt: unknown,
): ValidationResult {
  switch (item.engine) {
    case "mc": {
      ...
      if (attempt !== item.answer)
        return failure(["mc:wrong"], "Not the best choice.");
      return success();
    }
    case "cloze": {
      if (typeof attempt !== "string")
        return failure(["match:any"], "Type your answer.");
      const extra = item.acceptableAnswers ?? [];
      return matchAnyFuzzy([item.answer, ...extra], attempt);
    }
    case "builder":
      return validateTokenOrder(
        item.solution,
        attempt,
        item.solutionAlternates,
      );
    case "fixit":
    case "transform":
    case "timed": {
